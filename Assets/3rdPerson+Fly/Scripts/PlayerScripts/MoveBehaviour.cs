...
public class MoveBehaviour : GenericBehaviour
{
	...
	void Start()
	{
		...
		handDisk.GetComponent<MeshRenderer>().enabled = false;
	}
	...
	void Update()
	{
		...
		{
			if (backDisk.GetComponent<MeshRenderer>().enabled)
			{
				backDisk.GetComponent<MeshRenderer>().enabled = false;
				handDisk.GetComponent<MeshRenderer>().enabled = true;
			} else
			{
				backDisk.GetComponent<MeshRenderer>().enabled = true;
				handDisk.GetComponent<MeshRenderer>().enabled = false;
			}
		...
		//	diskAttack = Instantiate(GameObject.Find("Disk"), new Vector3(handDisk.transform.position.x + 1 / 10, handDisk.transform.position.y + 1/10, handDisk.transform.position.z + 1/10), handDisk.transform.rotation) as GameObject;
		//	//diskPosition = handDisk.transform.position;
		//	//handDisk.transform.position = Vector3.MoveTowards(transform.position, new Vector3(handDisk.transform.position.x + 1, handDisk.transform.position.y + 1 / 2, handDisk.transform.position.z), 1f);
		//	StartCoroutine(WaitForDiskColour());
		//}
		if (Input.GetMouseButtonDown(0) && handDisk.GetComponent<MeshRenderer>().enabled)
		{
			GameObject clone;
